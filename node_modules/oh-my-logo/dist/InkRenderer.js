...
function getGradientStyles(colors, count) {
    if (count <= 0) {
        return [];
    }
...
function renderBigText(text, font, letterSpacing) {
...
export function applyDirectionalGradient(text, colors, direction = 'horizontal') {
    ...
    switch (direction) {
        ...
        case 'diagonal': {
            ...
            return lines
                ...
                rowIndex += 1;
                return [...line]
                    .map((char, columnIndex) => applyStyle(char, styles[currentRowIndex + columnIndex]))
                    .join('');
            })
                .join('\n');
        }
        ...
        default: {
            const lines = cleanText.split('\n');
            const contentLineCount = lines.filter((line) => line.trim().length > 0).length;
...
const Logo = ({ text, colors, font = 'block', letterSpacing, direction = 'horizontal', }) => {
    const output = renderBigText(text, font, letterSpacing);
    return _jsx(Text, { children: applyDirectionalGradient(output, colors, direction) });
};
export function renderInkLogo(text, palette, options) {
    return new Promise((resolve) => {
        const { unmount } = render(_jsx(Logo, { text: text, colors: palette, font: options?.font, letterSpacing: options?.letterSpacing, direction: options?.direction }));
